function expression(cell,parms,i,j,im,jm) #Y0,η0,G0)
...
#******** 
y1  = cell[i,j].M;   # TP53,   stress, oxygen                   # oxygen
y2  = cell[i,j].E/(cell[i,j].E + parms.var17);   # ATR,                                     # estrogens
y3  = cell[i,j].E/(cell[i,j].E + parms.var17);   # ATM,                                     # estrogens
y4  = cell[i,j].E/(cell[i,j].E + parms.var17);   # BRCA1, Methylation --->                  # estrogens
y5  = cell[i,j].E/(cell[i,j].E + parms.var17);   # HER2, estrogens, growth factor, glucose  # estrogens
y6  = cell[i,j].M;   # MDM2, phosphorylation rates---->         # oxygen
y7  = cell[i,j].E/(cell[i,j].E + parms.var17);   # CHEK1                                    # estrogens
y8  = cell[i,j].M;   # AKT1                                     # oxygen
y9  = cell[i,j].M;   # P21                                      # oxygen
y10 = cell[i,j].E/(cell[i,j].E + parms.var17);   # CDK2                                     # estrogens
#***********
...
ftp = cell[i,j].geneexpression[11];
...
eta1 = cell[i,j].thresholdexpression[1]; 
eta2 = cell[i,j].thresholdexpression[2]; 
eta3 = cell[i,j].thresholdexpression[3]; 
eta4 = cell[i,j].thresholdexpression[4]; 
eta5 = cell[i,j].thresholdexpression[5]; 
eta6 = cell[i,j].thresholdexpression[6]; 
eta7 = cell[i,j].thresholdexpression[7]; 
eta8 = cell[i,j].thresholdexpression[8]; 
eta9 = cell[i,j].thresholdexpression[9]; 
eta10 = cell[i,j].thresholdexpression[10];
...
#Ecell = [x1 x2 x3 x4 x5 x6 x7 x8 x9 x10 ftp];
    ...
    cell[i+im,j+jm].geneexpression = [x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,ftp];
